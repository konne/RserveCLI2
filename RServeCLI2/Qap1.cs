...
namespace RserveCLI2
{
    ...
    internal class Qap1
    {
        ...
        public byte[] CommandReadStream( int cmd , IList<object> data ) =>
            Async.RunSynchronously(CommandReadStreamAsync(cmd, data));

        public async Task<byte[]> CommandReadStreamAsync(int cmd, IList<object> data)
        {
            long toConsume = await SubmitCommandAsync(cmd, data).DontContinueOnCapturedContext();
            ...
            {
                retrieved = await _socket.ReceiveAsync(tempBuf).DontContinueOnCapturedContext();
        ...
        {
            return Async.RunSynchronously(CommandAsync(cmd, data));
        }
        ...
        public async Task<List<object>> CommandAsync(int cmd, IList<object> data)
        {
            ...
            while ( toConsume > 0 )
            {
                ...
                int headerLength = 4;
                if (await _socket.ReceiveAsync(dhbuf, 4).DontContinueOnCapturedContext() != 4)
                {
                ...
                {
                    headerLength += 4;
                    if (await _socket.ReceiveAsync(dhbuf, 4, 4, SocketFlags.None).DontContinueOnCapturedContext() != 4)
                    {
                ...
                while ( receivedTotal < dlength )
                {
                    ...
                    var received = await _socket.ReceiveAsync(buf).DontContinueOnCapturedContext();
                    if ( received > 0 )
                    {
        ...
        private async Task<long> SubmitCommandAsync( int cmd , IList<object> data )
        {
            ...
            // Execute Command
            await _socket.SendAsync( sbuf.ToArray() ).DontContinueOnCapturedContext();

            // Read Response
            var hdrbuf = new byte[ 16 ];
            if ( await _socket.ReceiveAsync( hdrbuf ).DontContinueOnCapturedContext() != 16 )
            {
